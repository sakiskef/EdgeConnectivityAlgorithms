/*
 * Mansour-Schieber O(mn)-time edge connectivity algorithm (Algorithm I)
 */
#include <stdio.h>
#include <stdlib.h>
#include <iostream>
...
// find a vertex u in U s.t. outdegreeU[u]>=indegreeU[u]
inline int findODegreeU(int *outdegreeU, int *indegreeU, bool *Umark) {
	for (int u=1; u<=n; u++) {
		if ( (Umark[u]) && (outdegreeU[u] >= indegreeU[u]) ) return u;
	}
	return -1;
}

// find a vertex u in U s.t. indegreeU[u]>=outdegreeU[u]
inline int findIDegreeU(int* outdegreeU, int* indegreeU, bool* Umark) {
	for (int u = 1; u <= n; u++) {
		if ((Umark[u]) && (indegreeU[u] >= outdegreeU[u])) return u;
	}
	return -1;
}


inline void updateSetsO(int v, bool *Umark, queue <int> &rUq)
{
	rUq.push(v);
	for (int j = Gfirst[v]; j < Gfirst[v + 1]; j++) {
		int e = Gadj[j];
		...
		int y = get<1>(Edges[e]);  // to node
		if ((v == x) && Umark[y]) {
			Umark[y] = false; // y has an edge entering from O
...
inline void updateSetsI(int v, bool* Umark, queue <int> &rUq)
{
	rUq.push(v);
	for (int j = Gfirst[v]; j < Gfirst[v + 1]; j++) {
		int e = Gadj[j];
		...
		int y = get<1>(Edges[e]);  // to node
		if ((v == y) && Umark[x]) {
			Umark[x] = false; // x has an outgoing edge entering I
			//printf("\t vertex %d removed from U\n", x);
			rUq.push(x);
		}
	}
...
int MansourSchieber() {
	...
	updateSetsO(s, Umark, rUq);
	updateUdegrees(rUq, outdegreeU, indegreeU, &Usize); 
	int u = findODegreeU(outdegreeU, indegreeU, Umark);
	//printUdegree(Umark, outdegreeU, indegreeU);
	...
	while ( Usize > 0 )
	{
		...
		updateSetsO(u, Umark, rUq);
		updateUdegrees(rUq, outdegreeU, indegreeU, &Usize);
		u = findODegreeU(outdegreeU, indegreeU, Umark);
	}
	//printf("Usize = %d\n", Usize);
	printf("\t k = %d\n", k);
	...
	//printDegrees();
	//printUdegree(Umark, outdegreeU, indegreeU);
	//int* Iq = new int[n + 1]; // queue of ingoing dominating set I vertices 
	//int last_Iq; // last occupied position in Iq 
	//last_Iq = 0;
	//printf("vertex of max outdegree s = %d\n", s);
	//Iq[last_Iq++] = s;
	Imark[s] = true;
	...
	updateSetsI(s, Umark, rUq);
	updateUdegrees(rUq, outdegreeU, indegreeU, &Usize);
	u = findIDegreeU(outdegreeU, indegreeU, Umark);
